...
Ogni regola contiene da 1 a 10 definizioni di ricompensa. Una singola regola può assegnare più ricompense contemporaneamente — valute, badge o una combinazione. Esistono due tipi di ricompensa:

#### Ricompensa Valuta Virtuale

| Field | Type | Description |
|-------|------|-------------|
| `rewardType` | `VIRTUAL_CURRENCY` | Identifica questa come una ricompensa in valuta |
...
```json
// Statico: assegna sempre 100 punti
100

// Dinamico: assegna in base alla difficoltà del quiz
{
  "if": [
...
| Field | Type | Description |
|-------|------|-------------|
| `rewardType` | `BADGE` | Identifica questa come una ricompensa badge |
| `badgeConfigurationId` | nanoid | Quale badge assegnare |

Le ricompense badge sono più semplici di quelle in valuta: non c'è nessuna expression da valutare e nessuna modalità di riscatto. Quando la regola si attiva, il badge viene automaticamente assegnato all'utente. Il badge deve essere in stato `PUBLISHED` per poter essere assegnato.

Una singola regola può combinare entrambi i tipi — ad esempio, assegnare XP e un badge contemporaneamente al completamento di una missione:
...
| **rewards array** | 1–10 erogazioni per regola — `VIRTUAL_CURRENCY` (valuta + modalità + expression) o `BADGE` (solo ID badge) |
...
- **Dominio Badge**: le reward rule con `rewardType: BADGE` assegnano badge al completamento di missioni o learning path.
